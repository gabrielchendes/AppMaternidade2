  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-12 space-y-12 pb-32">
      <div className="space-y-2">
        ...
        <p className="text-gray-400">Gerencie suas informações e segurança da conta.</p>
      </div>

      {/* Avatar Section */}
      <div className="flex flex-col items-center gap-6">
        <div className="relative">
          <div className="w-40 h-40 rounded-full bg-zinc-800 border-4 border-primary/20 overflow-hidden flex items-center justify-center shadow-2xl">
            {avatarUrl ? (
        ...
        <button 
          ...
          className="flex items-center gap-2 px-6 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-white rounded-full border border-white/10 transition-all active:scale-95 shadow-lg group"
        >
          <Camera size={18} className="text-primary group-hover:scale-110 transition-transform" />
          <span className="text-sm font-bold">Trocar a foto</span>
        </button>
        ...
        <div className="text-center">
          <h2 className="text-2xl font-bold">{fullName || 'Seu Nome'}</h2>
        </div>
      </div>
      ...
      <section className="bg-zinc-900/50 rounded-2xl border border-white/10 p-6 space-y-6">
        ...
        <form onSubmit={handleUpdateProfile} className="space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-3 px-4 py-3 bg-black/40 rounded-xl border border-white/5 text-gray-500">
              <Mail size={18} />
              <span>{user.email}</span>
            </div>
            <p className="text-[10px] text-gray-500 italic">O e-mail não pode ser alterado.</p>
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-white text-black font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-gray-200 transition-all active:scale-[0.98] disabled:opacity-50"
          >
      ...
      <section className="bg-zinc-900/50 rounded-2xl border border-white/10 p-6 space-y-6">
        ...
        <form onSubmit={handleUpdatePassword} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-primary-hover text-white font-bold py-3 rounded-xl flex items-center justify-center gap-2 hover:bg-primary-hover transition-all active:scale-[0.98] disabled:opacity-50"
          >
